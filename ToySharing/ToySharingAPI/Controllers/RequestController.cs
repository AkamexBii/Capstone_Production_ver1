...
namespace ToySharingAPI.Controllers
{
    ...
    public class RequestsController : ControllerBase
    {
        ...
        private async Task CreateNotification(int userId, string content, int notificationType)
        {
        ...
        public async Task<IActionResult> ConfirmReturn(int requestId)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    await CreateNotification(
                        notificationUserId,
                        $"{role} đã xác nhận trả sản phẩm '{productName}'. Vui lòng xác nhận để hoàn thành yêu cầu.", 1
                    );
                }
        ...
        public async Task<ActionResult<RequestDTO>> CreateRequest([FromForm] CreateRequestDTO formData)
        {
            try
            {
                ...
                await CreateNotification(ownerId, $"{borrowerName} đã gửi yêu cầu mượn '{product.Name}' của bạn .", 2);
        ...
        public async Task<IActionResult> UpdateRequestStatus(int requestId, [FromBody] UpdateRequestStatusDTO requestDto)
        {
            ...
            try
            {
                ...
                else if (requestDto.NewStatus == 5)
                {
                    ...
                    var borrowerId = request.UserId;
                    var productName = request.Product.Name;
                    await CreateNotification(borrowerId, $"Yêu cầu mượn '{productName}' của bạn đã bị từ chối.", 5);
                }
                else
                {
        ...
        public async Task<IActionResult> MarkPickedUp(int requestId)
        {
            ...
            try
            {
                ...
                var ownerId = request.Product.UserId;
                var productName = request.Product.Name ?? "Sản phẩm không xác định";
                await CreateNotification(ownerId, $"Sản phẩm '{productName}' của bạn đã được người mượn lấy.", 1);
        ...
        public async Task<ActionResult<HistoryDTO>> ConfirmComplete(int requestId)
        {
            ...
            try
            {
                ...
                await CreateNotification(
                    ownerId,
                    $"{borrowerName} đã hoàn thành việc mượn sản phẩm '{productName}'. Vui lòng đánh giá người mượn trong lịch sử giao dịch.", 3
                );
        ...
        public async Task<IActionResult> RateHistory(int requestId, [FromBody] CompleteRequestDTO rateDto)
        {
            ...
            try
            {
                ...
                await CreateNotification(
                    history.UserId,
                    $"Bạn đã nhận được đánh giá từ chủ sở hữu '{productName}'.", 5
                );
