    ...
    public class PaymentsController : ControllerBase
    {
        ...
        public async Task<IActionResult> PaymentCallback([FromQuery] Dictionary<string, string> queryParams)
        {
            ...
            if (!isSuccess)
            {
                ...
                return Redirect($"https://toysharing.asia/payment-error?message={errorMessage}");
            }
            ...
            if (transaction != null)
            {
                ...
                var redirectUrl = $"https://toysharing.asia/payment-success" +
                                 $"?orderId={momoExecuteResponse.OrderId}" +
                                 $"&amount={transaction.Amount}" +
                                 $"&orderInfo={Uri.EscapeDataString(momoExecuteResponse.OrderInfo ?? "Thanh toán mượn đồ chơi")}" +
                                 $"&displayName={Uri.EscapeDataString(displayName)}";
            ...
            return Redirect($"https://toysharing.asia/payment-error?message={Uri.EscapeDataString("Không tìm thấy giao dịch")}");
        }
