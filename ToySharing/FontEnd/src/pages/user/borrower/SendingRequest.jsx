...
const SendingRequest = () => {
  ...
  const fetchRequests = async () => {
    try {
      ...
      const response = await axios.get(`${apiUrl}/api/Requests/toy-request`, {
        headers: {
  ...
  const visibleRequests = sortedRequests.slice(0, visibleItems);
  ...
  const handlePaymentClick = async (requestId) => {
    try {
      ...
      const response = await axios.post(`${apiUrl}/api/Payments/create`, paymentData, {
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  const handleConfirmPickedUp = async () => {
    try {
      ...
      await axios.put(
        `${apiUrl}/api/Requests/${selectedRequestId}/picked-up`,
        {},
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleConfirmReturn = async (requestId) => {
    try {
      ...
      await axios.put(
        `${apiUrl}/api/Requests/${requestId}/confirm-return`,
        {},
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleConfirmCancel = async () => {
    ...
    try {
      const token = getAuthToken();
      await axios.put(
        `${apiUrl}/api/Requests/${selectedRequestId}/cancel`,
        { reason: cancelReason },
        {
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      <Container fluid className="mt-4">
        <Row>
          ...
          <Col xs={12} md={10} className="main-content">
            ...
            {visibleRequests.length < sortedRequests.length && (
