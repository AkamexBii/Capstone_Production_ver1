...
const Message = () => {
  ...
  // Hàm chuyển đổi thời gian từ UTC sang UTC+7 (Việt Nam)
  const convertToVietnamTime = (utcDate) => {
    const date = new Date(utcDate);
    const vietnamTime = new Date(date.getTime() + 7 * 60 * 60 * 1000); // Thêm 7 tiếng
  ...
  useEffect(() => {
    ...
    const checkServerReady = async () => {
      try {
        await axios.get(`${apiUrl}/api/Conversations`, {
  ...
  const loadMessages = async (pageNum) => {
    try {
      const response = await axios.get(
        `${apiUrl}/api/conversations/${activeConversation.conversationId}/messages?page=${pageNum}&pageSize=10`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const response = await axios.post(
        `${apiUrl}/api/conversations/${activeConversation.conversationId}/messages`,
        { content: newMessage },
