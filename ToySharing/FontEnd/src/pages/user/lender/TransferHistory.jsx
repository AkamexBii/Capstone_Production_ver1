...
const TransferHistory = () => {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      try {
        ...
        const response = await axios.get(`${apiUrl}/api/Requests/borrow-history`, {
          headers: {
  ...
  useEffect(() => {
    ...
    uniquePartnerIds.forEach(async (partnerId) => {
      try {
        const token = getAuthToken();
        const response = await axios.get(`${apiUrl}/api/User/profile/${partnerId}`, {
  ...
  const handleSendRating = async () => {
    ...
    try {
      const token = getAuthToken();
      await axios.put(
        `${apiUrl}/api/Requests/history/${selectedRequestId}/rate`,
        {
          rating: rating,
