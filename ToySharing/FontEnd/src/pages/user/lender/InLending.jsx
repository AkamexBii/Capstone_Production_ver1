...
const InLending = () => {
  ...
  const fetchLendings = async () => {
    try {
      ...
      const response = await axios.get(`${apiUrl}/api/Requests/borrowing`, {
        headers: {
  ...
  useEffect(() => {
    ...
    uniqueLenderIds.forEach(async (lenderId) => {
      try {
        const token = getAuthToken();
        const response = await axios.get(`${apiUrl}/api/User/profile/${lenderId}`, {
  ...
  const handleMarkNotReturned = async (requestId) => {
    try {
      ...
      await axios.put(
        `${apiUrl}/api/Requests/${requestId}/mark-not-returned`,
        {},
        {
          headers: {
            Authorization: `Bearer ${token}`,
