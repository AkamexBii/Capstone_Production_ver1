...
const ListBorrowRequests = () => {
  ...
  const fetchRequests = async () => {
    try {
      ...
      const response = await axios.get(`${apiUrl}/api/Requests/pending`, {
        headers: {
  ...
  useEffect(() => {
    ...
    uniqueRequesterIds.forEach(async (requesterId) => {
      try {
        const token = getAuthToken();
        const response = await axios.get(`${apiUrl}/api/User/profile/${requesterId}`, {
  ...
  const handleConfirm = async () => {
    try {
      ...
      if (confirmAction === "accept") {
        ...
        await axios.put(
          `${apiUrl}/api/Requests/${selectedRequestId}/status`,
          { newStatus: 1 },
          {
            headers: {
              Authorization: `Bearer ${token}`,
        ...
        for (const req of otherRequests) {
          try {
            await axios.put(
              `${apiUrl}/api/Requests/${req.id}/status`,
              { newStatus: 5 },
              {
                headers: {
                  Authorization: `Bearer ${token}`,
