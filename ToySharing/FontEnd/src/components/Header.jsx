...
const Header = ({
  ...
  const fetchSenderName = async (senderId) => {
    ...
    try {
      const response = await axios.get(`${apiUrl}/api/User/profile/${senderId}`, {
  ...
  const handleMarkMessagesAsRead = async (conversationId) => {
    try {
      const response = await axios.get(
        `${apiUrl}/api/conversations/${conversationId}/messages?page=1&pageSize=100`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      ...
      await Promise.all(
        unreadMessages.map((msg) =>
          axios.put(
            `${apiUrl}/api/conversations/${conversationId}/messages/${msg.messageId}/read`,
            {},
            { headers: { Authorization: `Bearer ${token}` } }
          )
        )
